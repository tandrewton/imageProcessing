...
reducePolyTolerance = 0.001;
makeAMovie = 1;
savePlots = 0;
outputInterpolatedConfigurations = 1;
...
allWoundedPaths = [subPath_Abreu_Blanco];
...
for woundIt=1:length(allPaths)
    ...
    for ii = 1:length(reducePolyTolerance)
        ...
        for kk = 1:numberOfBoundaries
            ...
            %
            if outputInterpolatedConfigurations
                numVerts = 32;
                reducedPolygon = polyshape(P_reduced);
                pmeter = perimeter(reducedPolygon);
                P_resampled = resamplePolygonByLength(P_reduced,pmeter/numVerts);
                for ll=1:length(P_resampled(:,1))
                    fprintf(outputFileID, "%5d \t %.6f \t %.6f \n",kk-1,P_resampled(ll,2),P_resampled(ll,1));
                end
            end
        end
        if makeAMovie == 1
    ...
    for cellIt=1:length(shapeArraysTolerance(1,:))
        shapeError(:,cellIt) = 100*(shapeArraysTolerance(:,cellIt) - shapeArraysTolerance(1,cellIt))./shapeArraysTolerance(1,cellIt);
        areaError(:,cellIt) = 100*(areaArraysTolerance(:,cellIt) - areaArraysTolerance(1,cellIt))./areaArraysTolerance(1,cellIt);
    end
